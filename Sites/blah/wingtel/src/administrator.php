...
if(isset($_POST['submit'])){
  ...
  && !empty($_POST['departureTime']) && !empty($_POST['arrivalTime']) && !empty($_POST['airline']) && !empty($_POST['price'])){
    $origin = $_POST['origin'];
    ...
    $sql8 = "SELECT idAircraftManufacturer FROM AircraftManufacturer WHERE aircraftManufacturerName = '$airline'";
    $result7 = mysql_query($sql8, $connection);
    while($row4=mysql_fetch_array($result7)){
      $idAircraftManufacturer = $row4['idAircraftManufacturer'];
    }

    $sql9 = "SELECT idAircraft FROM Aircraft WHERE AircraftManufacturer_idAircraftManufacturer = '$idAircraftManufacturer' ORDER BY RAND() LIMIT 0,1";
    $result8 = mysql_query($sql9, $connection);
    while($row5=mysql_fetch_array($result8)){
      $idAircraft = $row5['idAircraft'];
    }

    $length = 6;
    $randomString = substr(str_shuffle("0123456789"), 0, $length);

    $sql4 = "INSERT INTO Flight(idFlight, Schedule_idSchedule, FlightStatus_idFlightStatus, Aircraft_idAircraft, Price)
    VALUES('$randomString', '$idSchedule', '1', '$idAircraft', '$price')";
...
<head>
    ...
    <script>
    	var table = document.getElementById('deleteList');
  ...
  <body id="page-top">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" id="mainNav">
      <div class="container">
        <a class="navbar-brand js-scroll-trigger" href="src/administrator.php">Wingtel (Admin)</a>
    ...
    <header class="masthead">
      <div class="container">
        ...
        <div id="adminHome">
          ...
          <?php
          $sql7 = "SELECT idFlight, departureDate, arrivalDate, departureTime, arrivalTime, origin, destination, flightStatusName, Price
          ...
          while($row5=mysql_fetch_array($result5)){
            echo "<form action=src/administrator.php method=post>";
            echo "<tr>";
        ...
        <div id="createFlight" style="display: none">
          ...
          <div class="row text-center">
            <div class="airlineText">Airline:</div>
            <div class="airline">
                <i class="fas fa-plane"></i>
              <select name="airline">
                <?php
                $sql8 = "SELECT * FROM AircraftManufacturer order by aircraftManufacturerName asc";
                $query2 = mysql_query($sql8, $connection);
                while($row3=mysql_fetch_array($query2)){
                ?>
                <option>
                  <?php echo $row3["aircraftManufacturerName"]; ?>
                </option>
                <?php
              }
                ?>
            ...
            <div class="price"><input type="text" name="price"></div>
          </div>
      ...
      <div id="updateFlight" style="display: none">
        <h2>Update Flight</h2>
        <?php
        $sql5 = "SELECT idFlight, departureDate, arrivalDate, departureTime, arrivalTime, origin, destination, flightStatusName, Price
        FROM Flight INNER JOIN Schedule ON Schedule_idSchedule = idSchedule INNER JOIN Direction
        ON Direction_idDirection = idDirection INNER JOIN FlightStatus ON FlightStatus_idFlightStatus = idFlightStatus";
      ...
      <div id="deleteFlight" style="display: none">
        <h2>Delete Flight</h2>
        <?php
        $sql6 = "SELECT idFlight, departureDate, arrivalDate, departureTime, arrivalTime, origin, destination, flightStatusName, Price
        FROM Flight INNER JOIN Schedule ON Schedule_idSchedule = idSchedule INNER JOIN Direction
        ON Direction_idDirection = idDirection INNER JOIN FlightStatus ON FlightStatus_idFlightStatus = idFlightStatus";
        $result4 = mysql_query($sql6, $connection);
        echo "<br>";
        echo "<table id=deleteList border=1>
        <tr>
        <th style=font-size:14px>Flight #</th>
        <th style=font-size:14px>Origin</th>
        <th style=font-size:14px>Destination</th>
        ...
        while($row4=mysql_fetch_array($result4)){
          ...
          echo "<td>"."<input readonly=readonly style=width:150px;text-align:center;font-size:12px;cursor:pointer type=text name=flightStatusName value=".$row4['flightStatusName']." </td>";
          echo "<td>"."<input readonly=readonly style=width:70px;text-align:center;font-size:12px;cursor:pointer type=text name=price value=".$row4['Price']." </td>";
          echo "</tr>";
        }
        echo "</table>";
        ?>
        <form action="src/delete.php" method="get">
          ...
          <input type="submit" name="delete" value="DELETE FLIGHT" class="btn-book" style="width: 150px">
        </form>
      </div>
